...
export default function Home() {
    ...
    return (
        <div className="bg-[#030014] text-white min-h-screen overflow-x-hidden">
            ...
            <section className="relative min-h-screen flex items-center justify-center pt-32 pb-20 px-6 md:px-12 lg:px-24">
                <CosmicBackground />

                <div className="relative z-10 w-full max-w-[1400px] mx-auto">
                    
                    {/* MOBILE: CENTERED VERTICAL LAYOUT */}
                    <div className="flex flex-col lg:hidden items-center text-center space-y-16">
                        ...
                        >
                            ...
                            >
                                ...
                                <span className="text-violet-400 font-mono text-[10px] uppercase tracking-widest">RGUKT Ongole 2026</span>
                            </motion.div>
                            ...
                            <h1 className="space-y-2">
                                ...
                                <motion.div
                                    ...
                                    className="text-[20vw] sm:text-[18vw] font-black tracking-tighter leading-none uppercase bg-gradient-to-r from-violet-400 via-purple-400 to-violet-400 bg-clip-text text-transparent gradient-animate"
                                    style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}
                                >
                                    2K26
                                </motion.div>
                        ...
                        >
                            <div className="glass-card p-8 sm:p-10 rounded-[2.5rem] relative shadow-2xl border-violet-500/20">
                                {/* Badge */}
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-violet-600 to-purple-600 px-5 py-2 rounded-full shadow-lg">
                                    <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.3em] text-white">Event Countdown</p>
                                </div>
                        ...
                        >
                            {/* Quote */}
                            <div className="relative px-6 py-6 glass-card rounded-2xl">
                            ...
                            {/* Date Badge */}
                            <div className="flex items-center justify-center gap-3 px-6 py-3 bg-violet-500/10 border border-violet-500/20 rounded-full w-fit mx-auto">
                                <Calendar size={16} className="text-violet-400" />
                                <span className="text-violet-400 font-bold text-xs uppercase tracking-widest">March 28 — 30, 2026</span>
                            </div>
                        </motion.div>
                        ...
                        >
                            <motion.a 
                                ...
                                whileTap={{ scale: 0.98 }}
                                className="inline-flex items-center gap-3 bg-gradient-to-r from-violet-600 to-purple-600 text-white px-8 py-4 rounded-full font-bold text-xs uppercase tracking-wider shadow-lg shadow-violet-500/30 transition-shadow hover:shadow-xl hover:shadow-violet-500/40"
                            >
                                <Download size={16} />
                    ...
                    <div className="hidden lg:grid grid-cols-2 gap-16 items-center">
                            ...
                            >
                                ...
                                <h1 className="space-y-3">
                                    ...
                                    <motion.div
                                        ...
                                        animate={{ opacity: 1, x: 0 }}
                                        transition={{ delay: 0.5, duration: 0.5 }}
                                        className="text-8xl xl:text-9xl font-black tracking-tighter leading-none uppercase bg-gradient-to-r from-violet-400 via-purple-400 to-violet-400 bg-clip-text text-transparent gradient-animate"
                                        style={{ fontFamily: 'system-ui, -apple-system, sans-serif' }}
                                    >
                                        2K26
                                    </motion.div>
                        ...
                        >
                            <div className="glass-card p-12 xl:p-16 rounded-[3rem] relative shadow-2xl border-violet-500/20 max-w-2xl w-full">
                                {/* Badge */}
                                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-violet-600 to-purple-600 px-6 py-2.5 rounded-full shadow-lg">
                                    <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-white">Event Countdown</p>
                                </div>
            ...
            <section className="py-20 px-6 border-y border-white/5 bg-white/[0.01]">
                <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-10">
                    {[
                        ...
                        >
                            <div className="text-violet-400 flex justify-center">{stat.icon}</div>
                            <h4 className="text-3xl md:text-5xl font-bold tracking-tight text-white">{stat.val}</h4>
                            <p className="text-[9px] md:text-[10px] uppercase text-gray-500 tracking-[0.4em] font-bold">{stat.label}</p>
                        </motion.div>
                    ))}
                </div>
            </section>

            {/* 4. ARENAS GRID - SIMPLIFIED */}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {deptMetadata.map((dept, index) => (
                        <Link key={dept.id} to={`/department/${dept.id}`}>
                            ...
                            >
                                {/* Gradient on hover */}
                                <div className={`absolute inset-0 bg-gradient-to-br ${dept.color} to-transparent opacity-0 group-hover:opacity-5 transition-opacity duration-500`} />
                                ...
                                {/* Content */}
                                <h3 className="text-3xl font-bold mb-4 group-hover:text-violet-400 transition-colors uppercase relative z-10">{dept.name}</h3>
                                <p className="text-gray-400 text-sm leading-relaxed mb-8 line-clamp-3 relative z-10">{dept.desc}</p>
                            ...
                            >
                                <div className="flex items-center justify-between mb-10 px-6 md:px-20 max-w-[1600px] mx-auto">
                                    <div className="flex items-center gap-6">
                                        <div className={`h-[2px] w-12 bg-gradient-to-r ${dept.color} to-violet-600`} />
                                        <h3 className="text-2xl md:text-4xl font-bold text-white uppercase">{dept.name}</h3>
                                    </div>
                                    <Link 
                                            ...
                                            >
                                                <div className="flex justify-between items-center mb-8">
                                                    <div className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-full text-[9px] font-bold text-white uppercase tracking-wider">
                                                        <Calendar size={12} className="text-violet-400" /> {formatDate(ev.date)}
                                                    </div>
                                                    <div className="h-2 w-2 rounded-full bg-violet-500 pulse-soft" />
                                                </div>
                                                
                                                <h4 className="text-3xl md:text-5xl font-bold mb-8 group-hover:text-violet-400 transition-colors line-clamp-1">{ev.name}</h4>
                                                ...
                                                <div className="mt-auto flex flex-wrap gap-4 text-gray-400 text-xs font-semibold uppercase tracking-wide">
                                                    <span className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/5">
                                                        <Clock size={14} className="text-violet-400" /> {ev.time}
                                                    </span>
                                                    <span className="flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/5">
                                                        <MapPin size={14} className="text-violet-400" /> {ev.venue}
                                                    </span>
                                                </div>
                                            </motion.div>
                                        </Link>
            ...
            {/* 6. DIRECTOR'S VISION - SIMPLIFIED */}
            <section className="py-48 px-6 text-center">
                ...
                >
                    <Quote size={60} className="text-violet-500/20 mx-auto mb-12" />
                </motion.div>
